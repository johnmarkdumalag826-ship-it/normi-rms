...
- Sign-in with email and password (passwords are stored hashed; sessions use signed tokens). Anyone can register (`POST /api/auth/register`), but it starts as "pending" until an Admin approves or rejects it (rejecting deletes the request); Admin then decides who can sign in. Nobody can sign up as an Admin. An Admin can create accounts directly too, but can never choose, see, or reset someone's password — only that person can, any time, with their current password (`PATCH /api/auth/change-password`). If someone forgets theirs, there is no recovery: an Admin deletes the old account so they can register again. Anyone signed in can also edit their own name and phone number (`PATCH /api/auth/me`) — email, role, department, course and account status stay Admin-only.
